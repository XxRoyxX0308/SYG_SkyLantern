...
var _using_sprite_a := true
var _hold_seconds := 5.0
var _transition_seconds := 1.2
var _generation := 0
...
func configure(carousel_config: Dictionary, stage_size: Vector2i) -> void:
	...
	if _sprite_a == null:
		_build_scene()
	_textures.clear()
	var paths := ConfigLoader.array_from(carousel_config.get("paths"), [])
	for path in paths:
	...
	_set_sprite_texture(_sprite_b, _textures[min(1, _textures.size() - 1)])
...
func _advance() -> void:
	...
	var current_sprite := _sprite_a if _using_sprite_a else _sprite_b
	var next_sprite := _sprite_b if _using_sprite_a else _sprite_a
