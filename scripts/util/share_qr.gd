...
static func make_texture(payload: String, size: Vector2i) -> Dictionary:
	...
	var codewords: Array[int] = _build_codewords(data)
	var matrix: Array = _build_matrix(codewords)
	var image: Image = _render_matrix(matrix, size)
	return {
...
static func _build_codewords(data: PackedByteArray) -> Array[int]:
	var bits: Array[int] = []
	_append_bits(bits, 0x4, 4)
	_append_bits(bits, data.size(), 8)
	for byte in data:
		_append_bits(bits, byte, 8)
	var capacity: int = DATA_CODEWORDS * 8
	var terminator: int = mini(4, capacity - bits.size())
...
static func _build_matrix(codewords: Array[int]) -> Array:
	var modules: Array[Array] = []
	var reserved: Array[Array] = []
	for _row in range(QR_SIZE):
		var module_row: Array[bool] = []
		var reserved_row: Array[bool] = []
...
static func _place_data(modules: Array, reserved: Array, codewords: Array[int]) -> void:
	var bits: Array[int] = []
	for byte in codewords:
		for shift in range(7, -1, -1):
			bits.append((byte >> shift) & 1)
	var bit_index: int = 0
	var right: int = QR_SIZE - 1
	var upward: bool = true
	while right >= 1:
		if right == 6:
			right -= 1
		for row_step in range(QR_SIZE):
			var y: int = QR_SIZE - 1 - row_step if upward else row_step
...
static func _draw_finder(modules: Array, reserved: Array, start_x: int, start_y: int) -> void:
	for y in range(-1, 8):
		for x in range(-1, 8):
			...
			var cell_y: int = start_y + y
			if cell_x < 0 or cell_x >= QR_SIZE or cell_y < 0 or cell_y >= QR_SIZE:
				continue
			var is_separator: bool = x == -1 or x == 7 or y == -1 or y == 7
			var is_border: bool = x == 0 or x == 6 or y == 0 or y == 6
			var is_center: bool = x >= 2 and x <= 4 and y >= 2 and y <= 4
...
static func _draw_format_bits(modules: Array, reserved: Array, mask: int) -> void:
	var ecl_bits: int = 1
	var data: int = (ecl_bits << 3) | mask
	var remainder: int = data << 10
	for bit in range(14, 9, -1):
		if ((remainder >> bit) & 1) != 0:
			remainder ^= 0x537 << (bit - 10)
	var format_bits: int = ((data << 10) | remainder) ^ 0x5412
...
static func _render_matrix(matrix: Array, size: Vector2i) -> Image:
	...
	var quiet_zone: int = 4
	var module_size: int = maxi(1, mini(size.x, size.y) / (QR_SIZE + quiet_zone * 2))
	var render_width: int = module_size * (QR_SIZE + quiet_zone * 2)
	var render_height: int = module_size * (QR_SIZE + quiet_zone * 2)
	var offset: Vector2i = Vector2i((size.x - render_width) / 2, (size.y - render_height) / 2)
