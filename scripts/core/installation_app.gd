...
@onready var _left_stage: Node = $LeftViewportContainer/LeftViewport/LeftStage
...
@onready var _right_stage: Node = $RightViewportContainer/RightViewport/RightStage

var _config := {}
var _selected_menu_ids := []
...
func _apply_window_config() -> void:
	var app_config := ConfigLoader.dictionary_from(_config.get("app", {}))
	var window_size := ConfigLoader.vector2i_from(app_config.get("window_size"), Vector2i(3840, 1080))
...
func _layout_viewports() -> void:
	var app_config := ConfigLoader.dictionary_from(_config.get("app", {}))
	var left_size := ConfigLoader.vector2i_from(app_config.get("left_resolution"), Vector2i(1920, 1080))
	var right_size := ConfigLoader.vector2i_from(app_config.get("right_resolution"), Vector2i(1920, 1080))
	var total_size := Vector2i(left_size.x + right_size.x, maxi(left_size.y, right_size.y))
...
func _on_drawing_confirmed(drawing_image: Image) -> void:
	var lantern_texture := PlaceholderFactory.compose_user_lantern(drawing_image)
...
func _on_screenshot_requested() -> void:
	var capture_result := await _capture_right_screen()
...
func _capture_right_screen() -> Dictionary:
	...
	var sharing := ConfigLoader.dictionary_from(_config.get("sharing", {}))
	var captures_dir := ConfigLoader.string_from(sharing.get("captures_dir"), "user://captures")
	var manifest_path := ConfigLoader.string_from(sharing.get("manifest_file"), captures_dir.path_join("index.json"))
	_ensure_directory(captures_dir)
	var manifest := _read_manifest(manifest_path)
	var token := _build_share_token(ConfigLoader.int_from(sharing.get("id_length"), 6))
	while manifest.has(token):
		token = _build_share_token(ConfigLoader.int_from(sharing.get("id_length"), 6))
	var user_path := captures_dir.path_join("%s.png" % token)
	var capture_image := _right_viewport.get_texture().get_image()
	var save_error := capture_image.save_png(user_path)
	if save_error != OK:
		return {
	...
	var payload := "%s%s" % [ConfigLoader.string_from(sharing.get("link_prefix"), "sc://"), token]
	manifest[token] = {
		"payload": payload,
	...
	var qr_result := ShareQr.make_texture(payload, Vector2i(360, 360))
	return {
...
func _build_share_token(length: int) -> String:
	var safe_length := clamp(length, 4, 8)
